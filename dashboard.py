...
def style_fig(fig, height=380, title=None):
    fig.update_layout(**PLOTLY_TEMPLATE_LAYOUT, height=height,
...
@st.cache_data
def load_data():
    patents = pd.read_csv(os.path.join(CLEAN_DIR, 'clean_patents.csv'),
                          dtype={'patent_id': str})
    inventors = pd.read_csv(os.path.join(CLEAN_DIR, 'clean_inventors.csv'),
                            dtype={'inventor_id': str})
    companies = pd.read_csv(os.path.join(CLEAN_DIR, 'clean_companies.csv'),
                            dtype={'company_id': str})
    relationships = pd.read_csv(os.path.join(CLEAN_DIR, 'relationships.csv'),
                                dtype={'patent_id': str, 'inventor_id': str, 'company_id': str},
                                low_memory=False)
    return patents, inventors, companies, relationships


patents, inventors, companies, relationships = load_data()


@st.cache_data
def build_aggregates(_patents, _inventors, _companies, _relationships):
    yearly = (_patents.dropna(subset=['year'])
              .groupby('year', as_index=False).size()
              .rename(columns={'size': 'patent_count'})
              .sort_values('year'))

    inv_rel = _relationships.dropna(subset=['inventor_id'])[['patent_id', 'inventor_id']]
    inv_counts = inv_rel.groupby('inventor_id').size().rename('patent_count').reset_index()
    top_inv = (_inventors.merge(inv_counts, on='inventor_id', how='inner')
               .sort_values('patent_count', ascending=False).head(25))

    co_rel = _relationships.dropna(subset=['company_id'])[['patent_id', 'company_id']]
    co_counts = co_rel.groupby('company_id').size().rename('patent_count').reset_index()
    top_co = (_companies.merge(co_counts, on='company_id', how='inner')
              .sort_values('patent_count', ascending=False).head(25))

    inv_country = (inv_rel.merge(_inventors[['inventor_id', 'country']], on='inventor_id', how='inner')
                   .dropna(subset=['country']))
    inv_country = inv_country[inv_country['country'] != '']
    countries_agg = (inv_country.groupby('country')['patent_id']
                     .nunique().rename('patent_count').reset_index()
                     .sort_values('patent_count', ascending=False))

    pat_year = _patents[['patent_id', 'year']].dropna()
    inv_country_year = (inv_country.merge(pat_year, on='patent_id', how='inner')
                        .groupby(['year', 'country'])['patent_id']
                        .nunique().rename('patent_count').reset_index())

    co_with_name = co_rel.merge(_companies[['company_id', 'name']], on='company_id', how='inner')
    co_year = (co_with_name.merge(pat_year, on='patent_id', how='inner')
               .groupby(['year', 'name'])['patent_id']
               .nunique().rename('patent_count').reset_index())

    recent = (_patents[['patent_id', 'title', 'year']]
              .sort_values(['year', 'patent_id'], ascending=[False, False])
              .head(300))

    totals = dict(
        patents=len(_patents),
        inventors=len(_inventors),
        companies=len(_companies),
        relationships=len(_relationships),
    )

    return totals, yearly, top_inv, top_co, countries_agg, inv_country_year, co_year, recent


totals, yearly, top_inventors, top_companies, countries, country_year, company_year, recent_patents = \
    build_aggregates(patents, inventors, companies, relationships)
...
with tab_diag:
    ...
    st.dataframe(conc[['name', 'patent_count', 'portfolio_share_%', 'cumulative_share_%']],
                 use_container_width=True, hide_index=True)
